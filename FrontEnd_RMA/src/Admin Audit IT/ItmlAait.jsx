      ...
      if (selectedYear) {
        try {
          ...
          if (response.data && response.data.payload && Array.isArray(response.data.payload.data)) {
            ...
            // Filter data API berdasarkan auditor "ITML"
            const ITMLAPIOrders = formattedData.filter(
              (order) => order.auditor === "DGCA"
            );
            const orderedITMLAPIOrders = updateOrderNumbers(ITMLAPIOrders);
  
            // Gabungkan dengan data dari localStorage
            setOrders(prevOrders => {
              const allOrders = [...prevOrders, ...orderedITMLAPIOrders];
